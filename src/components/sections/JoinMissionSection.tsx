...
import { useAccount } from 'wagmi'; // <--- 1. Impor useAccount dari wagmi

// Definisikan URL API Anda di sini atau sebagai environment variable
...
const JoinMissionSection: React.FC = () => {
  ...
  const { address: connectedAddress, isConnected } = useAccount(); // <--- 2. Dapatkan alamat yang terhubung dan status koneksi

  // 3. useEffect untuk mengisi otomatis alamat dompet jika terhubung
  useEffect(() => {
    if (isConnected && connectedAddress) {
      setWalletAddress(connectedAddress);
    } else {
      // Jika tidak terhubung, atau koneksi terputus, kosongkan field jika Anda mau
      // Namun, jika pengguna sudah mengetik sesuatu dan kemudian disconnect, mungkin lebih baik tidak mengosongkannya.
      // Untuk kasus ini, kita akan isi jika terhubung, dan biarkan pengguna yang mengosongkan/mengubah jika tidak.
      // Jika Anda ingin input dikosongkan saat disconnect:
      // setWalletAddress('');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.status === 'success') {
        ...
        setIsSubmitted(true);
        // Jangan kosongkan walletAddress di sini jika ingin tetap menampilkannya setelah submit
        // atau jika Anda ingin formnya bisa submit ulang dengan alamat yang sama (misalnya jika API gagal sebelumnya)
        // setWalletAddress(''); 
  ...
  useEffect(() => {
    // ... (IntersectionObserver tetap sama) ...
  ...
  const referralLink = useMemo(() => {
    ...
    return `${window.location.origin}${window.location.pathname.includes('index.html') ? window.location.pathname.replace('index.html', '') : window.location.pathname}?ref=${currentUserReferralCode}`;
  }, [currentUserReferralCode]);

  const handleCopyReferralLink = async () => {
    // ... (handleCopyReferralLink tetap sama) ...
  };
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto">
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-6">
                  ...
                  <input 
                    type="text" 
                    id="wallet"
                    value={walletAddress} // Value diambil dari state
                    onChange={(e) => setWalletAddress(e.target.value)} // Pengguna tetap bisa mengubah
                    ...
                    placeholder={isConnected && connectedAddress ? connectedAddress : "0x..."} // Placeholder bisa menampilkan alamat terhubung
                    required
                  />
                  {initialReferrerCode && (
                   ...
                   {/* Tambahkan indikasi jika alamat diambil dari dompet yang terhubung */}
              ...
              <div className="text-center py-8">
                ...
                <Button 
                  onClick={() => {
                    ...
                    setSubmissionMessage('');
                    // Isi lagi walletAddress jika user masih terhubung
                    if (isConnected && connectedAddress) {
                        setWalletAddress(connectedAddress);
                    } else {
                        setWalletAddress('');
          ...
          {/* ... (Kartu-kartu Connect, Pioneer Discovery, Mutual Prosperity tetap sama) ... */}
